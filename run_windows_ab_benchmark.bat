...
echo.
echo Running frozen EXE: 1 warm-up + 10 measured fixed-point runs...
start "" /wait "%BENCHMARK_EXE%" --no-dialog --run-count 11 --output-dir "%RESULTS_ROOT%\frozen_exe"
if errorlevel 1 goto :failed

echo Running Python source: 1 warm-up + 10 measured fixed-point runs...
"%PYTHON_EXE%" "%SOURCE_ROOT%\tools\windows_benchmark_runner.py" --no-dialog --run-count 11 --output-dir "%RESULTS_ROOT%\python_source"
if errorlevel 1 goto :failed
